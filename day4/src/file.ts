...
function checkIfOverlap(
  ...
  if (
    (number1 >= range1 && number1 <= range2) ||
    (number2 >= range1 && number2 <= range2)
  ) {
    return true;
  }
  return false;
...
function isOverlap(first: string, second: string) {
  let firstB = +first.split("-")[0];
  let firstE = +first.split("-")[1];
  let secondB = +second.split("-")[0];
  let secondE = +second.split("-")[1];
  if (
    checkIfOverlap(firstB, firstE, secondB, secondE) ||
    checkIfOverlap(secondB, secondE, firstB, firstE)
  ) {
    return true;
  }
  return false;
